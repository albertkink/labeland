...
const CreditsDropdown: React.FC = () => {
    ...
    try {
      ...
      const next =
        data && typeof data === "object"
          ...
          : null;
      if (typeof next !== "number") throw new Error("Invalid balance response.");
      setBalance(next);
    } catch (e) {
  ...
  const handleTopup = async () => {
    setError(null);
    if (!isAuthed) {
      setError("Please sign in to top up credits.");
      return;
    }
  ...
  return (
    <div ref={dropdownRef} className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="relative flex items-center justify-center h-10 gap-2 px-3 text-gray-700 rounded-lg hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800"
        aria-label="Credits"
      >
        <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-gray-100 text-xs font-bold text-gray-700 dark:bg-gray-800 dark:text-gray-200">
          $
        </span>
      ...
      {open ? (
        <div className="absolute right-0 z-50 mt-2 w-[320px] rounded-xl border border-gray-200 bg-white p-4 shadow-theme-lg dark:border-gray-800 dark:bg-gray-900">
          ...
          <div className="mt-4">
            ...
            <div className="mt-3 flex justify-end">
              <button
                ...
                onClick={handleTopup}
                disabled={isLoading}
                className="inline-flex h-10 items-center justify-center rounded-lg bg-brand-500 px-4 text-sm font-medium text-white shadow-theme-xs hover:bg-brand-600 disabled:opacity-60"
              >
                {isLoading ? "Redirecting..." : "Top up via Coinbase"}
              </button>
            </div>
          </div>
        </div>
...
const CartButton: React.FC<{ count: number }> = ({ count }) => {
  ...
  return (
    ...
    >
      ...
      >
        <path
          d="M2.5 4H3.57143L5.40171 14.1429C5.52442 14.8415 6.14552 15.3333 6.85714 15.3333H15.1429C15.8545 15.3333 16.4756 14.8415 16.5983 14.1429L18 6.33333H4.35714"
          stroke="currentColor"
          strokeWidth="1.5"
          strokeLinecap="round"
      ...
      {count > 0 ? (
        <span className="absolute -bottom-1 -right-1 inline-flex min-w-[18px] items-center justify-center rounded-full bg-brand-500 px-1.5 py-0.5 text-[11px] font-bold leading-none text-white shadow-theme-xs">
          {badgeText}
        </span>
      ) : null}
    </Link>
